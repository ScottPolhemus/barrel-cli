...
const themeCommand = require('@shopify/themekit').command
const fs = require('fs')
const config = require('./configure')
const Err = require('./error')
const minimatch = require('minimatch')

const theme = (action, flagObj, options, cb) => {
  let command = action === 'upload' ? 'deploy' : action;

  console.log({ action, flagObj, options })

  themeCommand(command, flagObj, options)
    .then(cb)
    .catch(cb)
}
